In der Interatkion zwischen Client und Server treten bei der Verarbeitung von Daten Fehler auf. Zur gezielten Behandlung der Fehler während der Entwicklung und zur gezielten Steuerung der Clientanwendung zur Laufzeit bietet der TI-Flow-Fachdienst eine Systematik an Fehlercodes an.

Diese Seite beschreibt dabei, welche Systematik für die Behandlung von Fehlern vorliegt und wie Fehlercodes systematisiert sind.

### Struktur der Fehlerrückgabe

Da der TI-Flow-Fachdienst FHIR als grundlegenedes Austauschformat verwendet, werden alle FHIR-Schnittstellen im Fehlerfall mit einer [TIFlow-OperationOutcome](./StructureDefinition-tiflow-operation-outcome.html) quittiert. Folgende Felder sind für den Client bei der Auswertung relevant:

|Feld|Bedeutung|
|---|---|
|issue.severity|Gibt an, um welche Schwere des Fehlers es sich handelt: Warnung oder Fehler|
|issue.code|Gibt an, um welche Art des Fehlers es sich handelt, bspw. ob es ein Validierungs- oder ein inhaltlicher Verarbeitungsfehler ist.|
|issue.details|Gibt eine genauere Beschreibung des Fehlers in einer kodierten Art und Weise an. Dabei sind textuelle und kodierte Inhalte vorhanden|
|issue.details.text|Gibt eine textuelle Repräsentation des Fehlers an.|
|issue.details.coding|Gibt einen Fehlercode, der in diesem IG beschrieben ist an|

<div><figcaption><strong>Tabelle: </strong>Definition der Felder von OperationOutcome</figcaption></div>
