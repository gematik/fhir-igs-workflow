...
# Load original file names from main to preserve underscores and exact stems.
raw = subprocess.check_output(
    ["git", "ls-tree", "-r", "main", "--name-only", "igs/rx/input/fsh"],
    text=True,
).strip().splitlines()
...
# Build mapping from current EFlowRx names back to GEM_ERP_* names.
name_map = {}
...
for path in root.rglob("*.fsh"):
    ...
    # Update definition headers based on mapping.
